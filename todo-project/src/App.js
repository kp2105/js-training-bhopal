...
import { setURL, getURL } from './urlpara';
...
function App() {
  ...
  //   const [filter, setfilter] = useState({ category: [], status: [], queryparam: '' });
  ...
  //       />
  //     </div>const [task, setTask] = useState(JSON.parse(localStorage.getItem("task")) || []);
  //   const [showList, setShowlist] = useState([...task]);
  //   const [message, setMessage] = useState('');
  //   const [tasktoEdit, settaskToEdit] = useState({});
  //   // const [filter, setfilter] = useState({});
  ...
  // let arr = jsondata.filter((t) => (t.isFOlder == true));
